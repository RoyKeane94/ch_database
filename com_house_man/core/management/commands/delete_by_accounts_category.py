...
EXCLUDED_ACCOUNTS_CATEGORIES = (
    "No Accounts Filed",
    "Dormant",
    "Accounts Type Not Available",
)
EXCLUDED_ACCOUNTS_CATEGORY_CONTAINS = ("micro entity",)
...
class Command(BaseCommand):
    help = (
        "Delete companies whose accounts category is dormant, unfiled, unavailable, "
        "or micro entity (including variants such as 'MICRO ENTITY ACCOUNTS')."
    )
    ...
    def handle(self, *args, **options):
        category_filter = Q()
        for category in EXCLUDED_ACCOUNTS_CATEGORIES:
            category_filter |= Q(accounts_category__iexact=category)
        for fragment in EXCLUDED_ACCOUNTS_CATEGORY_CONTAINS:
            category_filter |= Q(accounts_category__icontains=fragment)

        queryset = Company.objects.filter(category_filter)
        count = queryset.count()

        if count == 0:
            self.stdout.write("No companies matched the excluded accounts categories.")
            return

        breakdown = (
            queryset.values("accounts_category")
        ...
        for row in breakdown:
            label = row["accounts_category"] or "(blank)"
